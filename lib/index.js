...
var SiteSampler = function(settingsPathOrObject) {
  ...
  self.gs.on('result', function(results, options, timestamps) {
    timestamps.ms = timestamps.end - timestamps.start;
    options = R.merge(options, timestamps);

    log.info(options, 'Result from "' + options.url + '".');
    self.writeToDisk(results, options, settings);
  });

  self.gs.on('error', function(error, options, timestamps) {
    var ms = timestamps.end - timestamps.start;

    log.error(error, 'Error with results from goldwasher for "' + options.url + '". Duration: ' + ms + ' ms. Aborting write.');
