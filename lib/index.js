...
var SiteSampler = function(settingsPathOrObject) {
  ...
  self.gs = gs(settings.targets, settings.options);

  self.gs.on('running', function(options) {
    log.info(options, 'Sitesampler running.');
  });

  self.gs.on('result', function(results, options) {
    options.ms = options.end - options.start;
    log.info(options, 'Result from ' + options.url + ' in ' + options.ms + ' ms.');
    self.writeToDisk(results, options, settings);
  });

  self.gs.on('error', function(error, options) {
    log.error(error, 'Error with results from goldwasher for "' + options.url + '". Aborting write.');
...
SiteSampler.prototype.writeToDisk = function(results, options, settings) {
  var self = this;

  if (results.length > 0) {
    log.info({'url': options.url}, 'Writing results to chronostore...');

    cs.objectToStream(results)
      .pipe(cs.writeObject(settings.chronostore))
      .on('error', function(error) {
        log.error(error, 'Error when writing results to chronostore for "' + options.url + '".');
        if (settings.rethrowErrors) {
          self.emit('error', error);
        }
      })
      .on('finish', function() {
        ...
        self.emit('results', results, options);
      });
  } else {
    log.warn({'target': options}, 'No results from goldwasher. Aborting write.');
    self.emit('results', results, options);
  }
};
